...
my $theTreasure = "보물을 발견했다! 조용히 자리에서 일어나 연구실 밖 문 앞으로 향하자.\n".
"그 한쪽 구석에 그림이 있을 텐데, 그림 뒤를 보면 보물이 있을 것이다.\n".
"또, 항상 기억하도록 하자. 훌륭한 생물정보마술사는 포트 로더데일 합의를 준수하고,\n".
"자신들의 데이터/연구를 공유한다는 것을!";
...
"안타까운 점은 학부를 졸업했을 때 하필이면 대학원, 그것도 생명과에 가야겠다는 잘못된 선택을 한 바람에 내 기억이 예전 같지 못하다는 것이다.\n".
...
"방금 찾아낸 이 첫번째 단서를 그 디렉토리 안에 복사하고(cp), 디렉토리 안에 잘 들어갔는지 확인하자.\n\n이제 물의 탑으로 출발하라!\n";
...
"참고로 시약병 사이에 단서를 숨겨두려다 보니 가능한 작게 쪼그라뜨려서 집어넣어둘 수밖에 없었다. 읽으려면 gunzip을 써서 압축을 풀도록 하자.\n".
"단서를 가방(tote bag) 안에 복사(cp)해두는 것도 잊지 말 것!\n";

my $output3 = 'rumBarrel/northTwoSteps/westFourSteps/Clue03_O.txt';
my $OUT3 = &openOut($output3);

print $OUT3 "이 동네에 서식하는 다른 과학 덕후들이 컴퓨터를 어느 정도 알고 있다 보니, 이들이 우리가 남긴 단서를 쫓아올까봐 예쁜꼬마선충 폴리가  걱정하고 있다.\n".
"하지만 걱정하지 마라. 손은 눈보다 빠르니까. 이들을 속일 심산으로, 우린 99개의 빈 종이를 섞어두고, 딱 1개에만 단서를 적어 신문가판대(newsAgent) 안에 숨겨뒀다.\n".
"ls를 잘 사용할 줄 아는 우리만이 이 단서를 찾을 수 있을 것이다. 가판대 안에서 남쪽으로 다섯 걸음, 서쪽으로 한 걸음 내딛자.\n";
...
my $OUT4 = &openOut($output4);
print $OUT4 "이런, 세포 미디어 갈아주러 갈 시간이다.\n".
...
print $OUT5 "아주 예쁜 진주(Perl)가 해안(beach)에 떨어져 있다. 이 정도면 우릴 쫓아오는 사람들을 속일 수 있을 거다. 동쪽(east) 한 걸음, 남쪽으로 한 걸음.\n";
...
print $OUT8 "탭 만세! 탭이란 정말 어찌나 소중한지. 두 말할 필요가 없다. 탭 없었으면 파이펫 말고 키보드로도 관절염 생길 수 있다는 걸 배우게 됐겠지.".
...
"다시 한 번 해보자. 등대로 가서 서쪽으로 다섯 걸음, 다시 하지(summer solstice)에 최초로 비치는 빛이 만나는 지점을 향해 나아가라.\n";
...
print $OUT10 "좋아, 지금까지 뭔가를 만드는 일을 해봤다. 이제 박사과정들이 뭔가를 지우는 데 얼마나 일가견 있는지 한번 살펴보자.\n".
...
"이 파일에서 polly 라는 문자열을 검색하고(grep), 확인한 숫자를 \'key.txt\' 파일에 적어두자(vim).\n".
...
'휴지통이나 복구, 취소 따위 불가능하다. rm -rf 잘못 건들지 말어!  손모가지 날아가붕게.\n";'."\n";
